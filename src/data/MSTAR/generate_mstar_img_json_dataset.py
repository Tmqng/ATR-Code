...
class ErrorCounter:
    """Simple class-level counter for tracking processing errors."""
...
def get_partition_eoc2_cv(class_name, serial_num, depression):
    ...
    if depression == 17:
        if class_name in ["BMP2", "BRDM2", "BTR70", "T72"]:
            nominal_serials = [
                normalize_serial(s) for s in SOC_TRAIN_SERIALS[class_name]
            ]
            if serial_norm in nominal_serials:
                return (
                    f"EOC-2-CV/train/{class_name}",
                    target_name_eoc_2.index(class_name),
                )
...
def get_partition_outlier(class_name, serial_num, depression):
    ...
    if class_name in OUTLIER_KNOWN:
        ...
        if serial_norm in known_serials:
            if depression == 17:
                return (
                    f"OUTLIER/train/known/{class_name}",
                    target_name_confuser_rejection.index(class_name),
                )
            elif depression == 15:
                return (
                    f"OUTLIER/test/known/{class_name}",
                    target_name_confuser_rejection.index(class_name),
                )
    ...
    if class_name in OUTLIER_CONFUSER:
        ...
        if serial_norm in confuser_serials:
            if depression == 17:
                return (
                    f"OUTLIER/train/confuser/{class_name}",
                    target_name_confuser_rejection.index(class_name),
                )
            elif depression == 15:
                return (
                    f"OUTLIER/test/confuser/{class_name}",
                    target_name_confuser_rejection.index(class_name),
                )
...
def get_all_partitions(metadata, path_metadata, file_path):
    ...
    try:
        ...
        depression_str = (
            metadata.get("DesiredDepression")
            or metadata.get("MeasuredDepression")
            or str(path_metadata.get("depression_angle", 0))
        )
        ...
        for func in partition_funcs:
            partition = func(
                class_name, serial_num, depression
            )  # partition = (path, class_id)
            if partition:
                metadata_json = {
...
def create_directory_structure():
    ...
    # All unique classes
    all_classes = list(set(CLASS_MAP.values()))  # noqa: F841

    # SOC directories
    for split in ["train", "test"]:
        for class_name in SOC_TRAIN_SERIALS.keys():
            os.makedirs(
                os.path.join(OUTPUT_DIR, "SOC", split, class_name), exist_ok=True
            )
    ...
    for split in ["train", "test"]:
        for class_name in EOC1_SERIALS.keys():
            os.makedirs(
                os.path.join(OUTPUT_DIR, "EOC-1", split, class_name), exist_ok=True
            )
    ...
    for split in ["train", "test"]:
        for class_name in ["BMP2", "BRDM2", "BTR70", "T72"]:
            os.makedirs(
                os.path.join(OUTPUT_DIR, "EOC-2-CV", split, class_name), exist_ok=True
            )
    ...
    for split in ["train", "test"]:
        for class_name in ["BMP2", "BRDM2", "BTR70", "T72"]:
            os.makedirs(
                os.path.join(OUTPUT_DIR, "EOC-2-VV", split, class_name), exist_ok=True
            )
    ...
    for split in ["train", "test"]:
        for category in ["known", "confuser"]:
            for class_name in (
                OUTLIER_KNOWN.keys() if category == "known" else OUTLIER_CONFUSER.keys()
            ):
                os.makedirs(
                    os.path.join(OUTPUT_DIR, "OUTLIER", split, category, class_name),
                    exist_ok=True,
                )
...
def process_all_files(max_files=None):
    ...
    for root, dirs, files in os.walk(RAW_DATA_DIR):
        ...
        for file in files:
            ...
            try:
                ...
                if partitions:
                    ...
                    for partition_path, name_suffix, metadata_json in partitions:
                        ...
                        output_filepath_png = os.path.join(
                            OUTPUT_DIR, partition_path, output_filename_png
                        )
                        ...
                        output_filepath_json = os.path.join(
                            OUTPUT_DIR, partition_path, output_filename_json
                        )

                        # Save JSON metadata
                        with open(
                            output_filepath_json, mode="w", encoding="utf-8"
                        ) as f:
                        ...
                        if dataset == "OUTLIER":
                            ...
                            if (
                                class_name
                                not in dataset_counts[dataset][split][category]
                            ):
            ...
            if file_count % 100 == 0:
                ...
                print(
                    f"Progress: {file_count} files analyzed, {processed_count} processed"
                    f" ({total_assignments} total assignments), {skipped_count} skipped,"
                    f" {error_count} errors (Time: {elapsed:.1f}s)"
                )
    ...
    print("Training (17 deg) - Known:")
    for class_name, count in sorted(
        dataset_counts["OUTLIER"]["train"]["known"].items()
    ):
        print(f"  {class_name}: {count} images")
    print("Training (17 deg) - Confuser:")
    for class_name, count in sorted(
        dataset_counts["OUTLIER"]["train"]["confuser"].items()
    ):
        print(f"  {class_name}: {count} images")
    print("Test (15 deg) - Known:")
    for class_name, count in sorted(dataset_counts["OUTLIER"]["test"]["known"].items()):
        print(f"  {class_name}: {count} images")
    print("Test (15 deg) - Confuser:")
    for class_name, count in sorted(
        dataset_counts["OUTLIER"]["test"]["confuser"].items()
    ):
        print(f"  {class_name}: {count} images")
...
def test_single_file():
    ...
    if partitions:
        ...
        processed_img = process_image(mag_image, target_size=IMG_SIZE)
        print(
            f"Image processed: shape {processed_img.shape}, type {processed_img.dtype}"
        )
...
def create_mixed_dataset(train_proportion=0.7):
    ...
    if os.path.exists(MIXED_DIR):
        print("Cleaning existing MIXED data directory...")
...
def main():
    ...
    if choice == "1":
        ...
        confirm = (
            input("Are you sure you want to process ALL files? (y/N): ").strip().lower()
        )
        if confirm == "y":
